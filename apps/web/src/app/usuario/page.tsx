import type { Metadata } from "next";
import Link from "next/link";
import Header from "@/components/header";
import Footer from "@/components/footer";

const files = [
  {
    name: "contrato-proveedor.pdf",
    detected: "PDF",
    target: "DOCX",
    status: "Listo",
    availability: "Disponible 6 dias",
  },
  {
    name: "catalogo-producto.webp",
    detected: "Imagen",
    target: "PNG",
    status: "Procesando",
    availability: "En cola",
  },
  {
    name: "acta-reunion.docx",
    detected: "Documento",
    target: "PDF",
    status: "Listo",
    availability: "Disponible 2 dias",
  },
  {
    name: "entrevista-cliente.wav",
    detected: "Audio",
    target: "MP3",
    status: "Fallido",
    availability: "Requiere revision",
  },
];

const activity = [
  "Se detectaron 4 archivos esta semana.",
  "2 conversiones ya estan listas para descarga.",
  "1 archivo requiere volver a subirse por salida incompleta.",
];

export const metadata: Metadata = {
  title: "Mis Archivos | Reform Lab",
  description: "Historial reciente de archivos y conversiones del usuario.",
};

export default function UsuarioPage() {
  return (
    <div className="flex min-h-screen flex-col">
      <Header />

      <main className="flex-1 px-5 pb-10 pt-4 sm:px-8 sm:pb-12">
        <div className="mx-auto max-w-6xl">
          <div className="flex flex-col gap-4 border-b border-stone-200 pb-5 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <h1 className="text-[28px] font-semibold tracking-[-0.02em] text-stone-900">
                Mis archivos
              </h1>
              <p className="mt-1 text-sm text-stone-500">
                Consulta el formato detectado, la salida elegida y el estado de cada conversion reciente.
              </p>
            </div>

            <Link
              href="/"
              className="inline-flex h-10 items-center rounded-lg border border-stone-300 bg-white px-4 text-sm font-medium text-stone-700 transition-colors duration-150 hover:border-stone-400 hover:text-stone-900"
            >
              Nueva conversion
            </Link>
          </div>

          <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1.45fr)_minmax(280px,0.55fr)]">
            <section className="overflow-hidden rounded-xl border border-stone-200 bg-white">
              <table className="w-full border-collapse text-left">
                <thead className="bg-stone-50 text-xs font-medium text-stone-500">
                  <tr>
                    <th className="px-5 py-3">Archivo</th>
                    <th className="px-5 py-3">Detectado</th>
                    <th className="px-5 py-3">Salida</th>
                    <th className="px-5 py-3">Estado</th>
                    <th className="px-5 py-3">Disponibilidad</th>
                  </tr>
                </thead>
                <tbody>
                  {files.map((file) => (
                    <tr key={file.name} className="border-t border-stone-200 text-sm text-stone-700">
                      <td className="px-5 py-4 font-medium text-stone-900">{file.name}</td>
                      <td className="px-5 py-4">{file.detected}</td>
                      <td className="px-5 py-4">{file.target}</td>
                      <td className="px-5 py-4">{file.status}</td>
                      <td className="px-5 py-4 text-stone-500">{file.availability}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </section>

            <aside className="space-y-4">
              <section className="rounded-xl border border-stone-200 bg-white px-5 py-4">
                <h2 className="text-base font-semibold text-stone-900">Actividad reciente</h2>
                <ul className="mt-3 space-y-3 text-sm leading-6 text-stone-600">
                  {activity.map((item) => (
                    <li key={item}>{item}</li>
                  ))}
                </ul>
              </section>

              <section className="rounded-xl border border-stone-200 bg-white px-5 py-4">
                <h2 className="text-base font-semibold text-stone-900">Retencion</h2>
                <p className="mt-3 text-sm leading-6 text-stone-600">
                  Los artefactos completados permanecen disponibles durante 7 dias. Los archivos fallidos se eliminan antes.
                </p>
              </section>
            </aside>
          </div>
        </div>
      </main>

      <Footer />
    </div>
  );
}